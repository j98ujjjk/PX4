...
void BlockLocalPositionEstimator::updateHome()
{
	...
	mavlink_log_info(&_mavlink_log_pub, "[lpe] home: lat %5.0f, lon %5.0f, alt %5.0f", lat, lon, double(alt));
...
void BlockLocalPositionEstimator::correctSonar()
{
	...
	if (d < min_dist) {
		//mavlink_log_info(&_mavlink_log_pub, "[lpe] sonar min dist");
...
void BlockLocalPositionEstimator::correctBaro()
{
	...
	if (beta > BETA_TABLE[n_y_baro]) {
		if (_baroFault < FAULT_MINOR) {
			mavlink_log_info(&_mavlink_log_pub, "[lpe] baro fault, beta %5.2f", double(beta));
...
void BlockLocalPositionEstimator::correctLidar()
{
	...
	if (beta > BETA_TABLE[n_y_lidar]) {
		if (_lidarFault < FAULT_MINOR) {
			mavlink_log_info(&_mavlink_log_pub, "[lpe] lidar fault, beta %5.2f", double(beta));
